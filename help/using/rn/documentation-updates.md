...
## 2025 settembre {#september-2025}

* È stato chiarito che le chiamate API REST di Interactive Message Execution hanno un timeout di 60 secondi, con tentativi interni per garantire la consegna. [Ulteriori informazioni](../campaigns/trigger-campaigns.md)

* La pagina Raccolte elementi decisioning è stata aggiornata per chiarire il comportamento dell&#39;operatore **CONTAINS** durante la definizione delle regole. [Ulteriori informazioni](../experience-decisioning/collections.md)
...
* La documentazione relativa agli identificatori supplementari nei percorsi è stata aggiornata con i seguenti chiarimenti:
   ...
   * Dopo l’aggiunta di un identificatore supplementare a uno schema, è necessario creare un nuovo evento (per percorsi attivati da eventi) o un nuovo gruppo di campi (per percorsi Read audience). Le entità esistenti non vengono aggiornate automaticamente e non riconosceranno il nuovo identificatore.

   * Gli identificatori supplementari non vengono convalidati in base ai criteri DULE (Data Usage Labeling &amp; Enforcement, etichettatura e applicazione dell’uso dei dati) e non vengono considerati durante i controlli di governance dei dati nei percorsi.
