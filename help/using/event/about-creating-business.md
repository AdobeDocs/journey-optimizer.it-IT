...
>abstract="La configurazione dell’evento ti consente di definire le informazioni che Journey Optimizer riceverà sotto forma di eventi. Puoi utilizzare più eventi (in diversi passaggi di un percorso) e più percorsi possono utilizzare lo stesso evento. A differenza degli eventi unitari, gli eventi aziendali non sono collegati a un profilo specifico. Il tipo di ID evento è sempre basato su regole."
...
* Sono disponibili solo gli schemi di serie temporali. Gli schemi Eventi esperienza, eventi decisionali e eventi del passaggio al percorso non sono disponibili.
...
* Gli eventi aziendali possono essere eliminati solo come primo passaggio di un percorso.
* Quando si rilascia un evento di business come primo passaggio di un percorso, il tipo di pianificazione del percorso sarà &quot;evento di business&quot;.
...
* Quando esegui il test di un evento aziendale, devi trasmettere i parametri dell’evento e l’identificatore del profilo di test che verrà inserito nel percorso nel test. Inoltre, quando verifichi un percorso basato su eventi aziendali, puoi attivare solo un’entrata profilo singola. Vedi [questa sezione](../building-journeys/testing-the-journey.md#test-business). In modalità di test non è disponibile la modalità &quot;Vista codice&quot;.
* Cosa succede alle persone che sono attualmente in viaggio se arriva un nuovo evento di business? Si comporta come quando gli individui si trovano ancora in un percorso ricorrente quando si verifica una nuova ricorrenza. Il loro percorso è finito. Di conseguenza, gli esperti di marketing devono prestare attenzione a evitare di generare percorsi troppo lunghi se si aspettano frequenti eventi di business.
...
**Qual è il comportamento nella ricezione di un evento aziendale durante l’elaborazione del percorso?**

Gli eventi commerciali seguono le regole di rientro come per gli eventi unitari. Se un percorso consente il rientro, viene elaborato l’evento aziendale successivo.

**Quali sono le protezioni per evitare il sovraccarico dei segmenti materializzati?**

Nel caso di eventi aziendali on-shot, per un determinato percorso, i dati inviati dal primo lavoro evento vengono riutilizzati durante un intervallo di tempo di 1 ora. Per i percorsi pianificati, non è presente alcun protezionismo. Ulteriori informazioni sui segmenti nel [Documentazione del servizio di segmentazione di Adobe Experience Platform](https://experienceleague.adobe.com/docs/experience-platform/segmentation/home.html).
...
1. Definisci i campi dello schema e del payload: in questo punto è possibile selezionare i percorsi delle informazioni sull’evento (o payload) che si prevede di ricevere. Utilizzerai queste informazioni in un secondo momento del tuo percorso. Vedi [questa sezione](../event/about-creating-business.md#define-the-payload-fields).
...
1. Fai clic su **[!UICONTROL Save]**.
   ...
   L’evento è ora configurato e pronto per essere rilasciato in un percorso. Per ricevere gli eventi sono necessari ulteriori passaggi di configurazione. Ulteriori informazioni in [questa pagina](../event/additional-steps-to-send-events-to-journey.md).
