...
1. Nella sezione del menu DATA MANAGEMENT, fare clic su **[!UICONTROL Schemi]** e selezionare il pulsante **[!UICONTROL Crea schema]**.

   ![Menu Schemi con pulsante Crea schema](assets/test-profiles-0.png)

1. Seleziona **[!UICONTROL Standard]** come opzione di creazione dello schema.
1. Seleziona un tipo di schema, ad esempio **[!UICONTROL Profilo individuale]**, quindi fai clic su **Avanti**.
   ![Selezione del tipo di schema con l&#39;opzione Profilo individuale](assets/test-profiles-1.png)
...
1. Nell’elenco dei campi, fai clic sul campo che desideri definire come identità primaria.
   ![Elenco campi schema per la selezione dell&#39;identità primaria](assets/test-profiles-3.png)
1. Nel riquadro di destra **[!UICONTROL Proprietà campo]**, controllare le opzioni **[!UICONTROL Identità]** e **[!UICONTROL Identità primaria]** e selezionare uno spazio dei nomi. Se desideri che l&#39;identità primaria sia un indirizzo e-mail, scegli lo spazio dei nomi **[!UICONTROL E-mail]**. Fare clic su **[!UICONTROL Applica]**.
   ![Pannello delle proprietà del campo con le opzioni Identità e Identità primaria](assets/test-profiles-4bis.png)
...
1. Passa a **[!UICONTROL Set di dati]**, quindi fai clic su **[!UICONTROL Crea set di dati]**.
   ![Menu Set di dati con pulsante Crea set di dati](assets/test-profiles-6.png)
1. Scegli **[!UICONTROL Crea set di dati dallo schema]**.
   ![Crea set di dati dall&#39;opzione dello schema](assets/test-profiles-7.png)
1. Seleziona lo schema creato in precedenza e fai clic su **[!UICONTROL Avanti]**.
   ![Schermata di selezione dello schema per la creazione del set di dati](assets/test-profiles-8.png)
1. Scegli un nome e fai clic su **[!UICONTROL Fine]**.
   ![Finestra di dialogo Nome e completamento set di dati](assets/test-profiles-9.png)
1. Abilita l&#39;opzione **[!UICONTROL Profilo]**.
   ![Impostazioni del set di dati con opzione di profilo abilitata](assets/test-profiles-10.png)
...
Dalla pagina Home di [!DNL Adobe Journey Optimizer], puoi sfruttare il caso di utilizzo dei profili di test nel prodotto. Questo caso d’uso facilita la creazione di profili di test utilizzati per i percorsi di test prima della pubblicazione.

![Scheda del caso d&#39;uso dei profili di test nella home page](assets/use-cases-home.png)
...
1. Aggiungi una colonna per ogni campo obbligatorio. Assicurarsi di aggiungere il campo dell&#39;identità primaria (`personID` nell&#39;esempio precedente) e il campo `testProfile` impostato su `true`.
   ![File CSV con intestazioni di colonna che includono personID e testProfile](assets/test-profiles-11.png)
1. Aggiungi una riga per profilo e compila i valori per ciascun campo.
   ![File CSV con dati del profilo di test di esempio](assets/test-profiles-12.png)
1. Salva il foglio di calcolo come file csv. Assicurati che le virgole siano utilizzate come separatori.
1. Passa a [!DNL Adobe Experience Platform] **Flussi di lavoro**.
   ![Menu Flussi di lavoro in Adobe Experience Platform](assets/test-profiles-14.png)
1. Scegli **[!UICONTROL Mappa CSV a schema XDM]**, quindi fai clic su **Avvia**.
   ![Opzione di flusso di lavoro per mappatura CSV su schema XDM](assets/test-profiles-16.png)
1. Seleziona il set di dati in cui desideri importare i profili. Fai clic su **Avanti**.
   ![Schermata di selezione del set di dati per l&#39;importazione CSV](assets/test-profiles-17.png)
1. Fai clic su **Scegli i file** e seleziona il file CSV. Al caricamento del file, fare clic su **Avanti**.
   ![Schermata di caricamento file con pulsante Scegli file](assets/test-profiles-18.png)
1. Mappa i campi CSV di origine ai campi dello schema, quindi fai clic su **Fine**.
   ![Interfaccia di mappatura campi CSV con campi di origine e di destinazione](assets/test-profiles-19.png)
1. Viene avviata l’importazione dei dati. Lo stato passerà da **Elaborazione** a **Operazione completata**. Fai clic su **Anteprima set di dati**, in alto a destra.
   ![Stato di importazione che mostra il completamento con il pulsante Anteprima set di dati](assets/test-profiles-20.png)
1. Verifica che i profili di test siano stati aggiunti correttamente.
   ![Anteprima set di dati con profili di test importati](assets/test-profiles-21.png)
