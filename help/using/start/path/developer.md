...
>[!NOTE]
>
>**Ordine di implementazione:** [Amministratore](administrator.md) → [Data Engineer](data-engineer.md) → Sei qui: **Sviluppatore** → [Marketer](marketer.md)
>
>Assicurati che [schemi ed eventi di dati](data-engineer.md) siano configurati prima di implementare le integrazioni per dispositivi mobili e web.
...
Oltre all’implementazione degli SDK e dello streaming di eventi, puoi anche gestire Journey Optimizer a livello programmatico dai tuoi sistemi. La documentazione completa delle API, le specifiche OpenAPI e gli esempi di codice si trovano nel [portale per sviluppatori di di Journey Optimizer](https://developer.adobe.com/journey-optimizer-apis){target="_blank"}.

>[!NOTE]
>
>Tutte le integrazioni devono utilizzare l’autenticazione da server a server OAuth: il metodo JWT è obsoleto. [Configura l’autenticazione](https://developer.adobe.com/journey-optimizer-apis/references/authentication){target="_blank"}
...
* Le [campagne standard e a velocità effettiva elevata](../../campaigns/api-triggered-high-throughput.md) utilizzano endpoint diversi.

[Documentazione delle API](https://developer.adobe.com/journey-optimizer-apis/references/messaging){target="_blank"} · [Esempi di codice](https://developer.adobe.com/journey-optimizer-apis/references/messaging-samples){target="_blank"} · [Utilizzare campagne attivate da API](../../campaigns/api-triggered-campaigns.md)
...
[Documentazione dell’API capping](https://developer.adobe.com/journey-optimizer-apis/references/journeys-throttling){target="_blank"} · [Utilizzare l’API capping](../../configuration/capping.md) · [Utilizzare l’API throttling](../../configuration/throttling.md)
...
| Creare e gestire modelli di contenuto e frammenti da una pipeline esterna | [API dei contenuti](https://developer.adobe.com/journey-optimizer-apis/references/content){target="_blank"} · [Modelli](../../content-management/content-templates.md) · [Frammenti](../../content-management/fragments.md) |
| Recuperare e filtrare le campagne d’azione | [API delle campagne](https://developer.adobe.com/journey-optimizer-apis/references/campaigns-retrieve){target="_blank"} |
| Visualizzare l’anteprima delle campagne e inviare le bozze in modo programmatico | [API delle simulazioni](https://developer.adobe.com/journey-optimizer-apis/references/simulations){target="_blank"} |
...
Collabora con i [data engineer](data-engineer.md) sulle configurazioni di dati ed eventi. Ogni percorso che reagisce al comportamento dell’utente dipende dagli eventi inviati: il data engineer definisce gli schemi, mentre tu implementi il codice che li produce.

* Ottenere gli [schemi XDM](../../data/get-started-schemas.md) e le strutture di eventi necessari per l’implementazione
* Comprendere quali eventi inviare e il formato del payload richiesto: consulta [Utilizzare gli eventi del percorso](../../event/about-events.md)
* Verificare quali campi sono obbligatori e quali sono facoltativi in ciascun payload dell’evento e cosa accade nei percorsi in cui i campi previsti sono mancanti o in formato non valido: consulta [Requisiti dello schema](../../event/experience-event-schema.md#schema-requirements)
* Testare insieme la consegna degli eventi e l’acquisizione dei dati tramite [Adobe Experience Platform Assurance](https://experienceleague.adobe.com/docs/experience-platform/assurance/home.html?lang=it){target="_blank"}

>[!TAB Lavorare con gli amministratori]

Collabora con gli [amministratori](administrator.md) per l’accesso e le configurazioni dei canali. I percorsi possono raggiungere gli utenti solo tramite i canali configurati dall’amministratore: coordinati in anticipo in modo che il lavoro sull’SDK funzioni e la relativa configurazione rimangano sincronizzati.

* Fornire le specifiche API per le [azioni personalizzate](../../action/about-custom-action-configuration.md) che configureranno in Journey Optimizer
* Richiedere le autorizzazioni e le credenziali API necessarie tramite [Adobe Developer Console](https://developer.adobe.com){target="_blank"}
* Coordinare i requisiti di configurazione dei canali: certificati push per [iOS](../../push/push-configuration.md) e Android, impostazioni [push per il web](../../push/push-configuration-web.md), endpoint di [webhook SMS](../../mobile/mobile-webhook.md)
* Allinearsi alla strategia sandbox e agli ambienti di test prima di eseguire la [modalità di test del percorso](../../building-journeys/testing-the-journey.md)

>[!TAB Lavorare con i marketer]

Collabora con i [marketer](marketer.md) sui test e sulla progettazione del percorso. I marketer creano percorsi e contenuti che dipendono interamente dagli eventi inviati e dalle superfici esposte: maggiore è l’allineamento, più velocemente i percorsi vengono pubblicati.

* Rivedere le progettazioni del percorso in [Journey Optimizer](../../building-journeys/journey.md) insieme per capire quali interazioni utente devono attivare gli eventi e quali superfici necessitano di personalizzazione
* Implementare il tracciamento in modo che i marketer possano misurare le [prestazioni dei contenuti e il coinvolgimento degli utenti](../../reports/report-gs-cja.md)
* Eseguire [modalità di test percorso](../../building-journeys/testing-the-journey.md) insieme utilizzando i profili di test per convalidare il flusso completo end-to-end
* Risolvere i problemi relativi alla consegna dei messaggi, al rendering della personalizzazione o alle risposte delle [azioni personalizzate](../../action/action.md)
