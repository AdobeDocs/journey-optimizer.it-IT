...
**In questa pagina:** scopri come acquisire dati in Adobe Experience Platform da origini supportate e configurare un flusso di dati per le campagne orchestrate.
...
1. Dalla pagina **[!UICONTROL Dettagli set di dati]**, selezionare **[!UICONTROL Abilita acquisizione dati modifica]** per visualizzare solo i set di dati mappati a schemi relazionali e che includono sia una chiave primaria che un descrittore di versione.
   ...
   >[!IMPORTANT]
   >
   > Solo per **origini basate su file**, ogni riga nel file di dati deve includere una colonna `_change_request_type` con valori `U` (upsert) o `D` (delete). In assenza di questa colonna, il sistema non riconoscerà i dati come supporto del rilevamento delle modifiche e l’interruttore della campagna orchestrata non verrà visualizzato, impedendo la selezione del set di dati per il targeting.
