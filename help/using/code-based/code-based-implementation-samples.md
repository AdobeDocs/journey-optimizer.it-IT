...
feature_v2: id: d0a62d3c-b79e-47e4-929e-40ef3cffa037id: a984631b-2bae-4860-9b15-69c41a799dcbid: df64005d-8f9a-422e-ba4d-c6f6dc3454b4
...
**In questa pagina:** Esplora esempi di implementazione lato client, lato server e ibrida per recuperare, visualizzare e tenere traccia del contenuto delle esperienze basate su codice nell&#39;applicazione.
...
1. Per percorsi di esperienza e campagne basati su codice, gli eventi di interazione devono essere inviati manualmente per indicare quando un utente ha interagito con il contenuto. Questa operazione viene eseguita tramite il comando `sendEvent`.
   ...
   >[!IMPORTANT]
   >
   >Il campo `tokens` in `propositionAction` è fondamentale per il tracciamento e l&#39;attribuzione accurati in Adobe Journey Optimizer Decisioning. Questi token consentono:
   >* Attribuzione corretta dei clic per le attività decisionali
   >* Rapporti accurati sulle interazioni degli utenti con il contenuto delle decisioni
   >* Ottimizzazione delle prestazioni dell’offerta in base al coinvolgimento degli utenti
   >
   >I token si trovano in genere in `proposition.items[0].characteristics.tokens` e devono sempre essere inclusi durante il tracciamento delle interazioni dell&#39;utente con il contenuto decisionale.
...
## Eseguire il debug delle chiamate API di rete Edge con la garanzia Adobe Experience Platform {#debugging-edge-api-assurance}
