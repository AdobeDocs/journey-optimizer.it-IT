...
description: Scopri i codici di errore comuni in [!DNL Adobe Journey Optimizer] e come risolverli
...
subfeature_v2: id: bf4bee78-72fe-48e8-ba77-2ecb14c2827d
...
I codici di errore [!DNL Adobe Journey Optimizer] seguono un pattern di denominazione coerente che consente di identificare il componente e il tipo di problema:

* **Prefisso servizio**: indica quale servizio [!DNL Adobe Journey Optimizer] ha generato l&#39;errore.
Esempi: CJMPTS (servizio push/trasporto), CJMRT (runtime di Percorso), CJMMAS (servizio di authoring dei messaggi), CJMCMP (campagna), CJMTL (livello trasporto), CJMRPS (servizio di reporting/provisioning)
...
I codici di errore vengono visualizzati in diverse posizioni all&#39;interno di [!DNL Adobe Journey Optimizer]:
...
Utilizzare questa sezione per trovare i codici di errore raggruppati per servizio.
...
| **CJMPTS-1023-500** | Errore interno del server durante l’invio/elaborazione push (gateway di terze parti) | Errore temporaneo di malfunzionamento del cloud o errore sconosciuto del servizio | &#x200B;1. Verifica configurazione provider/canale<br/>2. Verificare lo stato del gateway di terze parti<br/>3. Riprova tra qualche minuto<br/>4. Controlla i registri per il contesto aggiuntivo <br/><br/>**Documentazione correlata**: [Notifiche push](../push/create-push.md) |
| **CJMPTS-1310-500** | Errore interno del servizio di rendering (anteprima o invio live) | Rendering del modello a valle non riuscito, in genere a causa di problemi di sintassi JSON/modello | &#x200B;1. Convalida la sintassi e la struttura del modello<br/>2. Verifica che tutte le variabili di personalizzazione siano valide<br/>3. Utilizza un payload di test per identificare il problema<br/>4. Semplifica la complessità del modello se necessario <br/><br/>**Documentazione correlata**: [Modelli di messaggio](../content-management/content-templates.md), [Sintassi Personalization](../personalization/personalization-syntax.md) |
...
| **CJMMAS-1603-403** | Non consentito durante l’aggiornamento/pubblicazione di un modello o di un predefinito | L’utente non dispone delle autorizzazioni/del ruolo richiesti o l’azione non è consentita nello stato corrente | &#x200B;1. Verificare che l&#39;utente disponga delle autorizzazioni appropriate (Gestione messaggi, Autore, ecc.)<br/>2. Verifica lo stato del predefinito/modello (bozza, pubblicato, archiviato)<br/>3. Richiedi l&#39;accesso all&#39;amministratore se necessario<br/>4. Rivedi assegnazioni profilo prodotto <br/><br/>**Documentazione correlata**: [Autorizzazioni](../administration/permissions.md), [Controllo dell&#39;accesso](../administration/permissions-overview.md) |
...
| Codice di errore | Descrizione | Causa principale | Risoluzione |
|------------|-------------|-----------|-----------|
| **CJMTL-010018-422** | &quot;Personalization non consentito nel nome di dominio&quot; durante il salvataggio/invio di contenuto | Una convalida eccessivamente rigida ha temporaneamente interrotto la personalizzazione del dominio href dinamico | &#x200B;1. Effettua il refactoring dei collegamenti se utilizzi variabili di dominio<br/>2. Verificare che la versione più recente di AJO sia in uso<br/>3. Ripetere l&#39;operazione<br/>4. Usa i domini statici se il problema persiste <br/><br/>**Documentazione correlata**: [Sintassi Personalization](../personalization/personalization-syntax.md), [Progettazione e-mail](../email/content-from-scratch.md) |
...
### CJMRPS: errori del servizio di reporting e provisioning {#cjmrps-errors}
...
7. **Ridimensiona quando necessario**: se l&#39;errore persiste dopo i seguenti passaggi di risoluzione, [contatta il supporto Adobe](../start/user-interface.md#support-ticket-guidelines) con il codice di errore completo, l&#39;ID richiesta (se disponibile), i passaggi da riprodurre e i dettagli di configurazione rilevanti.

## Best practice per evitare errori comuni {#best-practices}

Utilizza queste pratiche per ridurre gli errori evitabili e migliorare l’affidabilità.
