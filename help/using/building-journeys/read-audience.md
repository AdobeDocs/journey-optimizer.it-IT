...
**In questa pagina:** scopri come configurare l&#39;attività Read Audience per inserire in un percorso i profili di un pubblico di Adobe Experience Platform, su richiesta o secondo una pianificazione, e personalizzare il percorso di ciascun profilo.
...
Impostare: **Pubblico** (obbligatorio), **Spazio dei nomi** (obbligatorio), **Frequenza di lettura** (obbligatorio, predefinito 5.000/s) e **Pianificazione** (quando il percorso viene eseguito). È possibile aggiungere **Label** e **Identificatore supplementare**. I passaggi seguenti ti guidano attraverso ogni impostazione.

### Aggiungi attività e seleziona pubblico {#add-activity-and-select-audience}
...
1. Aggiungi un&#39;etichetta **[!UICONTROL Label]** all&#39;attività (facoltativo). Un’etichetta opzionale ti aiuta a identificare l’attività nei rapporti e nei registri in modalità di test.
...
### Identificatore supplementare {#read-audience-supplemental-id}
...
* Per informazioni sulla tempistica dello snapshot del pubblico, sulle finestre di completamento della segmentazione batch e su come garantire che il percorso venga sempre eseguito sui dati più recenti, vedere [Tempistica e propagazione dei dati](#timing-and-data-propagation). Per percorsi ricorrenti, è consigliabile abilitare l&#39;opzione **[!UICONTROL Trigger dopo la valutazione del pubblico in batch]** per ritardare automaticamente l&#39;esecuzione fino a quando l&#39;ultima istantanea del pubblico non è pronta. [Ulteriori informazioni](#schedule).
...
>abstract="Dopo la prima esecuzione, solo i nuovi profili aggiunti al pubblico entrano nel percorso."
...
>
>* **Non ricorrente** I percorsi Read audience passano automaticamente allo stato **Interrotto** una volta che l&#39;ultimo profilo attivo esce — a meno che il percorso non includa nodi che causano periodi di attesa (nodi di attesa, nodi di reazione o transizioni attivate da eventi), nel qual caso si applica il timeout globale standard di [91 giorni](journey-properties.md#global_timeout). [Ulteriori informazioni](end-journey.md#auto-stop-non-recurring)
...
## Targeting del pubblico in percorsi {#audience-targeting-in-journeys}

I percorsi basati sul pubblico iniziano sempre con un&#39;attività **Read Audience** per recuperare gli individui appartenenti a un pubblico [!DNL Adobe Experience Platform]. Tali profili vengono letti una volta o secondo una pianificazione ricorrente.
...
* [Best practice per percorsi Read Audience](https://experienceleaguecommunities.adobe.com/t5/journey-optimizer-blogs/mastering-read-audience-journeys-in-adobe-journey-optimizer-a/ba-p/761445){target="_blank"}: blog della community che tratta insidie comuni, discrepanze di conteggio e best practice collaudate.
...
* **Q: per quanto tempo un percorso Read Audience di una sola schermata rimane attivo?** — Passa allo stato Finito 91 giorni dopo l’esecuzione (timeout percorso globale).
